...
print("Preparation de la Heatmap...")
data = pd.read_csv(file_path, low_memory=False)

# Coordonnées géographiques pures sans NA
data_geo1 = data.dropna(subset=["consolidated_latitude", "consolidated_longitude"]).copy()

# Extraction des coordonnées avec un poids fixe de 1.0
heat_data = [[lat, lon, 1.0] for lat, lon in zip(data_geo1["consolidated_latitude"], data_geo1["consolidated_longitude"])]
...
output_heatmap = os.path.join(output_dir, "carte_heatmap.html")
carte1.save(output_heatmap)
...
#PREPARATION DES DONNEES
print("Generation du graphique de distribution...")

# Nettoyage strict des coordonnées et des puissances manquantes
data_geo2 = data.dropna(subset=["consolidated_latitude", "consolidated_longitude", "puissance_nominale"]).copy()
...
data_geo2['categorie_puissance'] = pd.cut(data_geo2['puissance_nominale'],
                                         bins=data_puiss,
                                         labels=data_label)
...
counts = data_geo2['categorie_puissance'].value_counts().reindex(data_label)
...
output_graphique = os.path.join(output_dir, "distribution_puissances.png")
plt.savefig(output_graphique, dpi=150)
plt.close()
print("Graphique enregistre")


#VISUALISATION CARTE
print("Génération de la carte de clusters...")
carte2 = folium.Map(
    location=[46.2276, 2.2137],
    zoom_start=6,
    tiles="CartoDB positron"
...
lats = data_geo2["consolidated_latitude"].values
lons = data_geo2["consolidated_longitude"].values
puissances = data_geo2["categorie_puissance"].astype(str).values
gratuites = data_geo2["gratuit"].astype(str).values  
pmrs = data_geo2["accessibilite_pmr"].astype(str).values
op = data_geo2["nom_operateur"].astype(str).values
...
carte2.get_root().html.add_child(branca.element.Element(html_final))


#SAUVEGARDE
output_clusters = os.path.join(output_dir, "carte_clusters.html")
carte2.save(output_clusters)
print("Carte de clusters enregistree")
print("Termine")